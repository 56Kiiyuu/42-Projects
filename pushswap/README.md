...
## Instructions
Compile the project at the root with `make`. Use `make clean` to remove object files, `make fclean` to remove the executable, and `make re` to recompile everything. Run the program as `./push_swap 3 2 1 42` or `./push_swap "3 2 1 42"`. To verify sorting: `./push_swap 3 2 1 42 | ./checker_linux 3 2 1 42` (should return `OK`).
